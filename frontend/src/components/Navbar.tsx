...
const Navbar = () => {
    ...
    return (
        <>
            <div className="bg-[var(--color-primary)] h-10 w-full md:h-12" /> {/* Red Top Bar - show on mobile too now */}
                    ...
                    <div className="bg-white rounded-full shadow-lg border border-gray-100 h-20 px-8 flex items-center justify-between gap-8 translate-y-[-10px]">
                        ...
                        <div className="flex-1 max-w-xl">
                            <form onSubmit={handleSearch} className="relative group flex items-center">
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border border-gray-100 rounded-full py-3.5 pl-12 pr-16 text-sm focus:ring-2 focus:ring-[var(--color-primary)]/10 focus:outline-none focus:bg-white transition-all placeholder-gray-400"
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                />
                                <button type="submit" className="absolute right-1.5 p-2.5 rounded-full bg-[var(--color-primary)] text-white hover:bg-black transition-colors shadow-sm">
                                    <Search className="w-5 h-5" />
                                </button>
                            </form>
                        </div>
                        ...
                        <div className="flex items-center space-x-6">
                            ...
                            <button className="relative text-gray-500 hover:text-[var(--color-primary)] transition-colors p-2">
                                ...
                                <span className="absolute top-1 right-1 bg-[var(--color-primary)] text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">0</span>
                            </button>

                            {/* Wishlist */}
                            <button className="text-gray-500 hover:text-[var(--color-primary)] transition-colors p-2">
                                ...
                                {isUserMenuOpen && (
                                    ...
                                    >
                                        ...
                                        <div className="py-1">
                                            ...
                                            {user?.role === 'admin' && (
                                                <Link to="/admin" className="flex items-center px-5 py-3 text-sm font-bold text-[var(--color-primary)] hover:bg-red-50 transition" onClick={() => setIsUserMenuOpen(false)}>
                                                    <ShieldAlert className="w-4 h-4 mr-3" /> Tableau d'administration
                                                </Link>
                                            )}
                                        </div>
                ...
                {(!hideSearch || isResults) && (
                    <div className="md:hidden px-3 py-2.5 flex items-center gap-2 border-b border-gray-100 bg-white">
                        ...
                        {isResults ? (
                            ...
                            >
                                <SlidersHorizontal className="w-5 h-5 text-[var(--color-secondary)]" />
                            </button>
                        ) : (
                            <button
                                onClick={() => setIsMenuOpen(true)}
                        ...
                        <form onSubmit={handleSearch} className="flex-1 relative flex items-center h-[42px]">
                            <input
                                ...
                                className="w-full h-full bg-gray-50 border border-gray-100 rounded-full py-0 pl-4 pr-12 text-[13px] focus:ring-2 focus:ring-[var(--color-primary)]/10 focus:outline-none placeholder-gray-400"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                            />
                            <button type="submit" className="absolute right-1 top-1 bottom-1 w-[38px] flex items-center justify-center rounded-full transition-colors shadow-sm" style={{ backgroundColor: 'var(--color-primary)', color: 'white' }}>
                        ...
                        <button className="text-gray-500 p-1 shrink-0 flex items-center justify-center h-[42px] relative">
                            ...
                            <span className="absolute top-1 right-0 bg-[var(--color-primary)] text-white text-[9px] font-bold w-3.5 h-3.5 rounded-full flex items-center justify-center">0</span>
                        </button>
                    </div>
                )}
            </nav>
                            ...
                            <div>
                                {user ? (
                                    ...
                                    <>
                                        ...
                                        <p className="text-xs text-gray-400">ou <Link to="/register" onClick={() => setIsMenuOpen(false)} className="font-bold text-[var(--color-primary)]">S'inscrire</Link></p>
                                    </>
                                )}
                            </div>
                        </div>
